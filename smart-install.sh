...
#   --check       resolver contract for embedders (e.g. optio): print exactly one
#                 stdout line — `kimi ok` (installed == latest) or
#                 `download <url>` (missing/stale) — and exit 0. The caller then
#                 downloads <url> with its own tooling. Mirrors the opencode fork.
...
case "$MODE" in
  url)
    ...
    ;;
  check)
    # Machine contract (one stdout line); human detail goes to stderr.
