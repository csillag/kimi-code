...
**退出 CLI**：输入框为空时按 `Ctrl-D`，或空闲状态下连按 `Ctrl-C` 两次，或输入 `/exit`。流式输出期间按 `Ctrl-C` 或 `Esc` 是中断当前轮次，不会退出程序。如果被中断的轮次已经输出了可见的 Assistant 文本，Kimi Code 会把这些文本保存在会话历史中，恢复会话后仍可作为上下文；尚未完成的 Thinking 输出不会保存。
